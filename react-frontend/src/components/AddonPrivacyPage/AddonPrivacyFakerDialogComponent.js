...
import addonPrivacyFakerFactory from "./addonPrivacyFakerFactory";
...
client.service("cbRefGitPrivate")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setprivacyidIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = addonPrivacyFakerFactory(props.user,fakerCount,projectIdIds,privacyidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonPrivacy").create(elem));
...
export default connect(mapState, mapDispatch)(AddonPrivacyFakerDialogComponent);
