...
privacyid: _entity?.privacyid?._id,
        };

        setLoading(true);
        try {
            
        await client.service("addonPrivacy").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info addonPrivacy updated successfully" });
...
const privacyidOptions = privacyid.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Addon Privacy" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="addonPrivacy-edit-dialog-component">
