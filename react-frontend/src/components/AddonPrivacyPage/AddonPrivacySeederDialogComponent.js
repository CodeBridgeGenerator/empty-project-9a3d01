...
import seederData from "./addonPrivacy.seeder.json";
...
const AddonPrivacySeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("addonPrivacy").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AddonPrivacySeederDialogComponent);
