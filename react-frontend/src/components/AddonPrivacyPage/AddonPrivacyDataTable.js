...
const AddonPrivacyDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.privacyid?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonPrivacy"});
...
<Column field="privacyid" header="Privacyid" body={dropdownTemplate1} filter={selectedFilterFields.includes("privacyid")} hidden={selectedHideFields?.includes("privacyid")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonPrivacy Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonPrivacy" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonPrivacyDataTable;