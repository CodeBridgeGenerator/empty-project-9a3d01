...
import cbRefDomainsFakerFactory from "./cbRefDomainsFakerFactory";
...
const CbRefDomainsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbRefDomainsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbRefDomains").create(elem));
...
export default connect(mapState, mapDispatch)(CbRefDomainsFakerDialogComponent);
