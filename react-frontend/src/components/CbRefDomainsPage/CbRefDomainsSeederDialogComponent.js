...
import seederData from "./cbRefDomains.seeder.json";
...
const CbRefDomainsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbRefDomains").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbRefDomainsSeederDialogComponent);
