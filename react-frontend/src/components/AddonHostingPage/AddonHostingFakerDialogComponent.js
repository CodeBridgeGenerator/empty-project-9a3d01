...
import addonHostingFakerFactory from "./addonHostingFakerFactory";
...
client.service("cbRefHosting")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        sethostingIdIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = addonHostingFakerFactory(props.user,fakerCount,projectIdIds,hostingIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonHosting").create(elem));
...
export default connect(mapState, mapDispatch)(AddonHostingFakerDialogComponent);
