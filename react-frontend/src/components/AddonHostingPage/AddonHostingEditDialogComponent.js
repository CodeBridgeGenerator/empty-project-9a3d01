...
hostingId: _entity?.hostingId?._id,
        };

        setLoading(true);
        try {
            
        await client.service("addonHosting").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info addonHosting updated successfully" });
...
const hostingIdOptions = hostingId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Addon Hosting" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="addonHosting-edit-dialog-component">
