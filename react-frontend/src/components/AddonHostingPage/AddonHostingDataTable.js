...
const AddonHostingDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.hostingId?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonHosting"});
...
<Column field="hostingId" header="HostingId" body={dropdownTemplate1} filter={selectedFilterFields.includes("hostingId")} hidden={selectedHideFields?.includes("hostingId")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonHosting Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonHosting" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonHostingDataTable;