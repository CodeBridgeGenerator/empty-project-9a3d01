...
const CbRefGitPrivateDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_booleanTemplate2 = (rowData, { rowIndex }) => <p >{String(rowData.value)}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "cbRefGitPrivate"});
...
<Column field="value" header="Value" body={p_booleanTemplate2} filter={selectedFilterFields.includes("value")} hidden={selectedHideFields?.includes("value")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload CbRefGitPrivate Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search CbRefGitPrivate" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CbRefGitPrivateDataTable;