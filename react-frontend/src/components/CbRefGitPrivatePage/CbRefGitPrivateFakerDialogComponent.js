...
import cbRefGitPrivateFakerFactory from "./cbRefGitPrivateFakerFactory";
...
const CbRefGitPrivateFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbRefGitPrivateFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbRefGitPrivate").create(elem));
...
export default connect(mapState, mapDispatch)(CbRefGitPrivateFakerDialogComponent);
