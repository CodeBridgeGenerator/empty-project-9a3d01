...
import seederData from "./cbRefGitPrivate.seeder.json";
...
const CbRefGitPrivateSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbRefGitPrivate").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbRefGitPrivateSeederDialogComponent);
