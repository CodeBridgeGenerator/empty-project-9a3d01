...
const AddonPackageDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.packageId?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonPackage"});
...
<Column field="packageId" header="PackageId" body={dropdownTemplate1} filter={selectedFilterFields.includes("packageId")} hidden={selectedHideFields?.includes("packageId")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonPackage Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonPackage" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonPackageDataTable;