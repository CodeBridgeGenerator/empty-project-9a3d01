...
import addonPackageFakerFactory from "./addonPackageFakerFactory";
...
const AddonPackageFakerDialogComponent = (props) => {
...
client.service("cbRefPackages")
    ...
    const onRun = async () => {
        let fakeData = addonPackageFakerFactory(props.user,fakerCount,projectIdIds,packageIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonPackage").create(elem));
...
export default connect(mapState, mapDispatch)(AddonPackageFakerDialogComponent);
