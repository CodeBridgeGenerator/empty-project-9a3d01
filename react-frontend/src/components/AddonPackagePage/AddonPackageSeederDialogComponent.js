...
import seederData from "./addonPackage.seeder.json";
...
const AddonPackageSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("addonPackage").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AddonPackageSeederDialogComponent);
