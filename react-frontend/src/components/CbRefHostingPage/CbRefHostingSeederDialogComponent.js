...
import seederData from "./cbRefHosting.seeder.json";
...
const CbRefHostingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbRefHosting").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbRefHostingSeederDialogComponent);
