...
const CbRefHostingDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const imageTemplate7 = (rowData, { rowIndex }) => <Image src={rowData.image}  alt="Image" height="60px" />
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "cbRefHosting"});
...
<Column field="image" header="Image" body={imageTemplate7} filter={selectedFilterFields.includes("image")} hidden={selectedHideFields?.includes("image")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload CbRefHosting Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search CbRefHosting" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CbRefHostingDataTable;