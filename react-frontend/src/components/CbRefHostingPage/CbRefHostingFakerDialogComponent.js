...
import cbRefHostingFakerFactory from "./cbRefHostingFakerFactory";
...
const CbRefHostingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbRefHostingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbRefHosting").create(elem));
...
export default connect(mapState, mapDispatch)(CbRefHostingFakerDialogComponent);
