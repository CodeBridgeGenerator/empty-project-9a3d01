...
import seederData from "./cbRefPackages.seeder.json";
...
const CbRefPackagesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbRefPackages").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbRefPackagesSeederDialogComponent);
