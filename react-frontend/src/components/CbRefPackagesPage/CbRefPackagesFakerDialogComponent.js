...
import cbRefPackagesFakerFactory from "./cbRefPackagesFakerFactory";
...
const CbRefPackagesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbRefPackagesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbRefPackages").create(elem));
...
export default connect(mapState, mapDispatch)(CbRefPackagesFakerDialogComponent);
