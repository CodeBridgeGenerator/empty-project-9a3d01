...
import cbRefSupportFakerFactory from "./cbRefSupportFakerFactory";
...
const CbRefSupportFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbRefSupportFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbRefSupport").create(elem));
...
export default connect(mapState, mapDispatch)(CbRefSupportFakerDialogComponent);
