...
import seederData from "./cbRefSupport.seeder.json";
...
const CbRefSupportSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbRefSupport").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbRefSupportSeederDialogComponent);
