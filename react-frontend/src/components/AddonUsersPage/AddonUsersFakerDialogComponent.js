...
import addonUsersFakerFactory from "./addonUsersFakerFactory";
...
const AddonUsersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = addonUsersFakerFactory(props.user,fakerCount,projectIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonUsers").create(elem));
...
export default connect(mapState, mapDispatch)(AddonUsersFakerDialogComponent);
