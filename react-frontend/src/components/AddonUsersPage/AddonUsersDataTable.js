...
const AddonUsersDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate1 = (rowData, { rowIndex }) => <p >{rowData.number}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonUsers"});
...
<Column field="number" header="Number" body={p_numberTemplate1} filter={selectedFilterFields.includes("number")} hidden={selectedHideFields?.includes("number")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonUsers Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonUsers" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonUsersDataTable;