...
import seederData from "./cbRefDiscountCode.seeder.json";
...
const CbRefDiscountCodeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("cbRefDiscountCode").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CbRefDiscountCodeSeederDialogComponent);
