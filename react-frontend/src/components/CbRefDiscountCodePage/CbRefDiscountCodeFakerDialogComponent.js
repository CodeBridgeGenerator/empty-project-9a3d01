...
import cbRefDiscountCodeFakerFactory from "./cbRefDiscountCodeFakerFactory";
...
const CbRefDiscountCodeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cbRefDiscountCodeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cbRefDiscountCode").create(elem));
...
export default connect(mapState, mapDispatch)(CbRefDiscountCodeFakerDialogComponent);
