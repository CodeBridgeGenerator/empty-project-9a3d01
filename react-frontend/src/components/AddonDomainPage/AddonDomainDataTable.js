...
const AddonDomainDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.domainid?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonDomain"});
...
<Column field="domainid" header="Domainid" body={dropdownTemplate1} filter={selectedFilterFields.includes("domainid")} hidden={selectedHideFields?.includes("domainid")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonDomain Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonDomain" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonDomainDataTable;