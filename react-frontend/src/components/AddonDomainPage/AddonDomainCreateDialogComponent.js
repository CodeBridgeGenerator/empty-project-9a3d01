...
const [domainid, setDomainid] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [projectId,domainid], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            projectId: _entity?.projectId?._id,domainid: _entity?.domainid?._id,
        ...
        const result = await client.service("addonDomain").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Addon Domain updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Addon Domain" });
        }
        setLoading(false);
    };
...
const domainidOptions = domainid.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Addon Domain" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="addonDomain-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["domainid"]) ? (
              <p className="m-0" key="error-domainid">
                {error["domainid"]}
              </p>
            ) : null}
          </small>
