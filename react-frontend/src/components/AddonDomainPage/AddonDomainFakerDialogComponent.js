...
import addonDomainFakerFactory from "./addonDomainFakerFactory";
...
const AddonDomainFakerDialogComponent = (props) => {
...
client.service("cbRefDomains")
    ...
    const onRun = async () => {
        let fakeData = addonDomainFakerFactory(props.user,fakerCount,projectIdIds,domainidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonDomain").create(elem));
...
export default connect(mapState, mapDispatch)(AddonDomainFakerDialogComponent);
