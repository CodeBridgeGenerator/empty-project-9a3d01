...
import seederData from "./addonDiscount.seeder.json";
...
const AddonDiscountSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("addonDiscount").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AddonDiscountSeederDialogComponent);
