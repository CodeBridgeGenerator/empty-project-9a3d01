...
const AddonDiscountDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.projectId?.name}</p>
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.discountId?.code}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonDiscount"});
...
<Column field="projectId" header="ProjectId" body={dropdownTemplate0} filter={selectedFilterFields.includes("projectId")} hidden={selectedHideFields?.includes("projectId")}  style={{ minWidth: "8rem" }} />
<Column field="discountId" header="DiscountId" body={dropdownTemplate1} filter={selectedFilterFields.includes("discountId")} hidden={selectedHideFields?.includes("discountId")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonDiscount Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonDiscount" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonDiscountDataTable;