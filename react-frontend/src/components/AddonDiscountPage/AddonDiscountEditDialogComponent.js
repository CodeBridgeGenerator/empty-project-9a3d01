...
discountId: _entity?.discountId?._id,
        };

        setLoading(true);
        try {
            
        await client.service("addonDiscount").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info addonDiscount updated successfully" });
...
const discountIdOptions = discountId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Addon Discount" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="addonDiscount-edit-dialog-component">
