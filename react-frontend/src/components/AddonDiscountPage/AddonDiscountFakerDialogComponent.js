...
import addonDiscountFakerFactory from "./addonDiscountFakerFactory";
...
client.service("cbRefDiscountCode")
                        .find({ query: { $select: ["_id","code"] } })
                        .then((data) => {
                        setdiscountIdIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = addonDiscountFakerFactory(props.user,fakerCount,projectIdIds,discountIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonDiscount").create(elem));
...
export default connect(mapState, mapDispatch)(AddonDiscountFakerDialogComponent);
