...
import addonSupportFakerFactory from "./addonSupportFakerFactory";
...
client.service("cbRefSupport")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setsupportidIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = addonSupportFakerFactory(props.user,fakerCount,projectidIds,supportidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addonSupport").create(elem));
...
export default connect(mapState, mapDispatch)(AddonSupportFakerDialogComponent);
