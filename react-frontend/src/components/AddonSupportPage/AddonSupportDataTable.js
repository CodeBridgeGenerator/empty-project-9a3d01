...
const AddonSupportDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.supportid?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "addonSupport"});
...
<Column field="supportid" header="Supportid" body={dropdownTemplate1} filter={selectedFilterFields.includes("supportid")} hidden={selectedHideFields?.includes("supportid")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload AddonSupport Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search AddonSupport" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default AddonSupportDataTable;